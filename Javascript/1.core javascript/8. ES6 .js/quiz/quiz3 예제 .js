...
const sortedtrade = traders.slice().sort((a, b) => a.value - b.value);

// 중간값 거래정보 찾기
// 배열의 요소 수가 홀수면 정확한 중간인덱스가 나오는데 짝수면
// 중앙 두값의 평균을 중간값으로 한다.
// 가운데 인덱스 찾기
const middleIndex = Math.floor(sortedtrade.length / 2);

let middleIndexInfo;
if (sortedtrade.length % 2 === 1) {
  // 배열의 요소 수가 홀수인 경우 정확한 중간 인덱스를 지정
  // 정확한 중간 인덱스를 지정
  middleIndexInfo = sortedtrade[middleIndex];
} else {
  //짝수인 경우
  middleIndexInfo = [sortedtrade[middleIndex - 1], sortedtrade[middleIndex]];
}
console.log(middleIndexInfo);
